...
public class JSONNotificationContentBuilder extends NotificationContent.Builder {
  ...
  protected  boolean getSticky(JSONObject payload) {
    try {
      return payload.has(STICKY_KEY) && payload.getBoolean(STICKY_KEY);
    } catch (JSONException e) {
      Log.e("expo-notifications", "Could not have parsed a boolean sticky value passed in notification.");
      return false;
    }
  }
}
