...
import {
    MentionData, MentionPartType, Part, PartType, Position, RegexMatchResult, Suggestion
} from '../types';
...
const generateValueWithAddedSuggestion = (
  ...
  const triggerPartIndex = mentionType.insertWithoutReplacingCurrentPart
    ? currentPart.position.end
    : currentPart.text.lastIndexOf(
        mentionType.trigger,
  ...
  if (partTypes.length === 0) {
    ...
    for (let i = 0; i < matches.length; i++) {
      ...
      if (
        ...
        result.groups.trigger !== partType.trigger
      ) {
